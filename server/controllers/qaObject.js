...
async function createQaObject(req, res) {
    try {
        const { question, answer, described_day_date } = req.body;
        const { _id } = req.user;
        const newQA = new qaObject({
            question,
            answer,
            user_id: _id,
            described_day_date: described_day_date || Date.now
        });
        result = await newQA.save()
        res.json({ message: "object created successfully", newQA })
    } catch (error) {
        console.log(error)
        res.status(500).json({ error: "internal server error" })
    }
...
async function deleteQaObject(req, res){
    try {
        const { id } = req.params        
    } catch (error) {
