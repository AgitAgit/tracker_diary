const DailyPage = require('../models/dailyPage')
...
//Create a new empty dailyPage, or update if it already exists
router.post('/', authUser, async (req, res) => {
    try {
        const { user } = req;
        const { title, described_day_date } = req.body;
        const date = described_day_date ? new Date(described_day_date) : Date.now()
        const newDailyPage = new DailyPage({
            user_id: user.id,
            title: title,
            free_text: text,
